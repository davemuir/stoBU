...
if($_POST['type']){
	...
	$fm = new FileMaker("streetsTo_List.fmp12","199.38.217.147","Admin","streetsto");
	$rec = $fm->newFindCommand("categories");
	$rec->addFindCriterion('pid',$_POST['type']);
	$result = $rec->execute();
	$resp = $result->getRecords();		
	 $super = [];
	foreach ($resp as $record) {
	$super[$record->getField('id')] =  $record->getField('locationName');
	}
	...
	include 'FileMaker.php';	
	$lid = $_POST['lid'];
	$fm = new FileMaker("streetsto_menus.fmp12","199.38.217.147","Admin","streetsto");
	...
	$fm = new FileMaker("streetsto_location.fmp12","199.38.217.147","Admin","streetsto");
