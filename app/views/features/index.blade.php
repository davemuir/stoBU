...
$(document).ready(function(){
	...
	$(".bootstrap-switch").click(function(){
		...
		if($(this).hasClass('bootstrap-switch-off')){
			...
			var xhr = new XMLHttpRequest();
          	xhr.open('GET', 'http://198.1.96.237/beacons/state/inactive/'+perkID, false);
          	xhr.send(null);
            if (xhr.status == 200) {
			...
			var xhr = new XMLHttpRequest();
          	xhr.open('GET', 'http://198.1.96.237/beacons/state/active/'+perkID, false);
          	xhr.send(null);
            if (xhr.status == 200) {
              console.log(xhr.response);
            }
	...
	$(".orderSelect").change(function(e,oldValue) {
			...
			if(check == false){
				var xhr = new XMLHttpRequest();
		      	xhr.open("GET", "http://198.1.96.237/beacons/order/"+perk+"/"+order, false);
		      	xhr.send(null);
		        if (xhr.status == 200) {
		          console.log(xhr.response);
		        }
